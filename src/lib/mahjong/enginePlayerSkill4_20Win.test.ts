...
function createState(): GameState {
  const state = createInitialGameState(
    ...
    {
      enemyId: "enemy-1",
      equippedSkills: [{ id: "4-20", level: 1 }]
    }
  );
...
function exchange(state: GameState): GameState {
  return activatePlayerSkill4_20(
    state,
    ["4-20-outgoing"],
    () => 0
  );
...
describe("4-20の発動から交換和了まで", () => {
  it("元のツモ牌を交換しても取得した萬子で和了できる", () => {
